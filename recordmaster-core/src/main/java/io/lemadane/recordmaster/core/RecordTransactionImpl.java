    ...
    public void commit() {
        ...
        try {
            ...
            for (Map.Entry<String, TableChangeSet> entry : changeSet.getAllTableChanges().entrySet()) {
                ...
                for (io.lemadane.recordmaster.Record record : tc.getInserts().values()) {
                ...
                for (io.lemadane.recordmaster.Record record : tc.getUpdates().values()) {
                    byte[] recBytes = BinaryCodec.serialize(record);
                    ByteArrayOutputStream baos = new ByteArrayOutputStream();
                    DataOutputStream dos = new DataOutputStream(baos);
                    dos.writeUTF(tableName);
            ...
            for (Map.Entry<String, TableChangeSet> entry : changeSet.getAllTableChanges().entrySet()) {
                ...
                for (Object key : tc.getDeletes()) {
                    ...
                    io.lemadane.recordmaster.Record oldRecord = null;
                    if (oldTableState != null) {
